...
@property (atomic, strong, readwrite) NSMutableData *receiveData;
...
{
    if (!(self = [super init])) return nil; // LCOV_EXCL_LINE
...
//	_session = [NSURLSession sessionWithConfiguration:config delegate:nil delegateQueue:nil];
    _session = XCDShareSession.shareInst.sessionMan.session;
...
- (void)startNextRequest
{
    if (self.eventLabels.count == 0) {
        if (self.requestType == XCDYouTubeRequestTypeWatchPage || self.webpage) [self finishWithError];
        else [self startWatchPageRequest];
    } else {
...
//    self.dataTask = [XCDShareSession.shareInst.sessionMan downloadTaskWithRequest:request progress:nil destination:^NSURL *_Nonnull (NSURL *_Nonnull targetPath, NSURLResponse *_Nonnull response) {
//        NSLog(@"target :%@", targetPath);
//        NSURL *documentsDirectoryURL = [[NSFileManager defaultManager] URLForDirectory:NSDocumentDirectory inDomain:NSUserDomainMask appropriateForURL:nil create:NO error:nil];
//        return [documentsDirectoryURL URLByAppendingPathComponent:@(NSDate.date.timeIntervalSince1970).stringValue];
//    } completionHandler:^(NSURLResponse *_Nonnull response, NSURL *_Nullable filePath, NSError *_Nullable error) {
//        if (self.isCancelled) return;
//        if (error) [self handleConnectionError:error requestType:requestType];
//        else [self handleConnectionSuccessWithData:[NSData dataWithContentsOfURL:filePath] response:response requestType:requestType];
//    }];
//    self.dataTask.taskDescription = @(NSDate.date.timeIntervalSince1970).stringValue;
    self.dataTask = [XCDShareSession.shareInst.sessionMan dataTaskWithRequest:request uploadProgress:nil downloadProgress:nil completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
        if (self.isCancelled)
            return;
        ...
        if (error)
            ...
            [self handleConnectionSuccessWithData:responseObject response:response requestType:requestType];
    }];
...
- (void)handleVideoInfoResponseWithInfo:(NSDictionary *)info response:(NSURLResponse *)response
{
    ...
    if (video) {
        ...
        if ([error.domain isEqual:XCDYouTubeVideoErrorDomain] && error.code == XCDYouTubeErrorUseCipherSignature) {
            ...
            if (error.userInfo[NSLocalizedDescriptionKey]) self.youTubeError = error;
...
- (void)handleDashManifestWithXMLString:(NSString *)XMLString response:(NSURLResponse *)response
{
    ...
    if (dashhManifestStreamURLs) [self.lastSuccessfulVideo mergeDashManifestStreamURLs:dashhManifestStreamURLs];
